...
- **Residual block:** conv (kernel 8) → BN → ReLU → conv (5) → BN → ReLU → conv (3) → BN, plus a shortcut (1×1 conv + BN when the channel count changes, otherwise BN only), then ReLU after the addition. Stride 1 and `"same"` padding, so the length stays 7500.
...
| Epochs | 1500 | code |
| Batch size | `min(N_train / 10, 16)` → 16 for both tasks | code |
...
- Config cell near the top: `numberSeeds = 1` to start, `10` for the full run.
...
- **Run time is an estimate:** about 3.8 billion multiply-adds per trial on the forward pass; roughly 0.5–1 s per epoch on the RTX 3090, so 15–25 min per run and about 10–17 h for 4 configs × 10 seeds. Not measured.
